...
*FEATURES
    #Criar dama
    #Passar jogada automaticamente
    #Scoreboard
    #mostrar peças fora do jogo
    #Timer
    #Menu de config do jogo
...
class Jogo{
    constructor(jogadorPretas,jogadorVermelhas, tabuleiro){
        ...
        this.pecasPretasRestantes = 12;
        this.pecasVermelhasRestantes = 12;
    }
    ...
    // Funçoes para alterar o placar de peças restantes
    atualizaPlacar(){
        document.getElementById('placar1').innerText = `Peças restantes: ${this.pecasPretasRestantes}`;
        document.getElementById('placar2').innerText = `Peças restantes: ${this.pecasVermelhasRestantes}`;
    }

    // Como uso jogadorDaRodada como referencia ao chamar essa função na 124,
    // a logica aqui é inversa =S
    decrementaPecasRestantes(jogador) {
        if (jogador === this.jogadorVermelhas) {
            this.pecasPretasRestantes--;
        } else if (jogador === this.jogadorPretas) {
            this.pecasVermelhasRestantes--;
        }
        this.atualizaPlacar();
    }
        ...
        this.div.onclick = () => {
            if(this.movimentoValido()){
                ...
                if(pecaSelecionada.tipoMovimento==TipoMovimento.SIMPLES){
                    ...
                    // Chamei a função para decrementar peças. Usando jogadorDaRodada como referencia
                    this.tabuleiro.jogo.decrementaPecasRestantes(this.tabuleiro.jogo.jogadorDaRodada);
                }else{
...
jogadorPretas.div.classList.add("jogadorSelecionado");//
jogo.inicializa();